    ...
    reqCheckUpdate.then((response) => {
        if (response.status == 200) {
            if (JSON.parse(response.body)[0].tag_name > VERSION) {
                console.warn(`[NIA V4.5] The current plugin package is not the latest version, the current version is: ${VERSION}, Github on the release of the latest version is: ${JSON.parse(response.body)[0].tag_name}. link: https://github.com/NIANIANKNIA/NiaServer-Core/releases/tag/${JSON.parse(response.body)[0].tag_name}`);
