...
//     fs.GetJSONFileData("new_shop_data.json").then((result) => {
//         //文件不存在
//         if (result === 0) {
//             fs.CreateNewJsonFile("new_shop_data.json",{"sell_data":[],"recycle_data":[]}).then((result) => {
//                 if (result === "success") {
//                     warn("【商店系统】在获取商店数据文件 new_shop_data.json 时发现文件不存在，已成功创建")
//                 } else if (result === -1) {
//                     error("【商店系统】在创建玩家货币数据文件 new_shop_data.json 时与NIAHttpBOT连接失败")
//                 }
//             });
//         } else if (result === -1) {
//             error("【商店系统】在获取商店数据文件 new_shop_data.json 时与NIAHttpBOT连接失败")
//         } else {
//             SellData = result.sell_data;
//             RecycleData = result.recycle_data;
//             log(`【商店系统】商店系统数据文件 new_shop_data.json 获取成功`)
//         }
//     })
// })
