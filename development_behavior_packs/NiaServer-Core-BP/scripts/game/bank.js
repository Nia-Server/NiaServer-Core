...
let n_coin_data = {};
system.run(() => {
    n_coin_data = world.getDynamicProperty("n_coin_data");
    if (n_coin_data == undefined) {
        n_coin_data = {};
        world.setDynamicProperty("n_coin_data", "{}");
    } else {
        n_coin_data = JSON.parse(n_coin_data);
    }
})
...
export const BankGUI = {
    Main(player) {
        ...
        .show(player).then((response) => {
            ...
            if (response.selection == 1){
                // this.Ranking(player);
                player.sendMessage(" §c开发中功能，敬请期待");
                return;
            }
    ...
    BuyNCoinInfo(player) {
        ...
        .body("§e请在购买§c虚拟货币N币（以下简称“N币”）§e前仔细阅读以下条款：\n\n" +
            ...
            "其中§c前三小时§e，§cY§e将在区间§c[0.8,1.2]§e中随机分配取值；§c三小时过后Y§e将在区间§c[0,0.8+Z]§e (§cZ = ( 当前存入的小时总数向上取整后 - 3) / 2§e)范围之内进行取值，"+
            ...
            "其中§c前三小时§e，§cY§e将在区间§c[0.8,1.2]§e中随机分配取值；§c三小时过后Y§e将在区间§c[0,0.9+Z]§e (§cZ = ( 当前存入的小时总数向上取整后 - 3)§e)范围之内进行取值，"+
            "且§c每过一整个小时§e，§cY§c取值§c0.9§e以上的概率将降低§c8%%§e，对应的§c1§e以下的概率将升高§c8%%§e\n\n"+
        ...
        .show(player).then((response) => {
            ...
            if (response.selection == 0) {
                ...
                n_coin_data[player.id] = {
                    ...
                    "status": "normal",
                    "save_time": time,
                    "buy_coin_num": Number(n_coin_num),
                    "now_coin_num": Number(n_coin_num),
                    "insurance": insurance,
                    "buy_time": Date.now(),
                    "change_log" : []
                }
                world.setDynamicProperty("n_coin_data", JSON.stringify(n_coin_data));
    ...
    CheckNCoin(player) {
        //获取N币数据
        let player_n_coin_data = JSON.parse(world.getDynamicProperty("n_coin_data"))[player.id];
        //检查是否有N币数据
        ...
        if (player_n_coin_data.status != "complete") {
            //计算剩余结算次数
            let total_count = 0;
            ...
            if (total_time < save_time || save_time == -1) {
                ...
                player_n_coin_data.status = "finish";
            }
            ...
            if (player_n_coin_data.change_log.length <= 15) {
                ...
                for (let i = 0; i < count_3h; i++) {
                    ...
                    if (player_n_coin_data.now_coin_num / player_n_coin_data.buy_coin_num <= 0.0001) player_n_coin_data.status = "break";
                }
            }
            //剩余次数的结算（三个小时后的）
            ...
            for (let i = 1; i <= total_count; i++) {
                if (player_n_coin_data.status == "break") break;
                let Z = Math.ceil(i / 5);
                if (player_n_coin_data.type == "活期存法") Z = Z * 2 / 3;
                log(`Z值为${Z}`);
                //计算Y值
                //计算各区间的概率调整
                //基础概率是50%(0.5)对半分
                ...
                if (Math.random() < base_rate) {
                    ...
                    //在[0.8, 0.8+Z] 区间内随机取值
                    if (player_n_coin_data.type == "活期存法") random_num = Math.random() * Z + 0.8;
                    //在 [0.9, 0.9+Z] 区间内随机取值
        ...
        switch (player_n_coin_data.status) {
            case "normal":
                CheckNCoinForm.body(`§6您当前的N币数据如下：\n\n` +
                `§e当前持有状态：§c结算进行中\n` +
                `§e买入初始数量：§c${player_n_coin_data.buy_coin_num}\n` +
                `§e当前持有类型：§c${player_n_coin_data.type}\n` +
                `§e当前持有数量：§c${player_n_coin_data.now_coin_num.toFixed(2)}\n` +
                `§e当前持有时间：§c${total_time.toFixed(2)}h\n`)
                for (let i = 0; i < player_n_coin_data.change_log.length; i++) {
                    CheckNCoinForm.divider();
                    CheckNCoinForm.label(`§e第§c${i+1}§e次结算后数量： ` +
                    `§c${player_n_coin_data.change_log[i].after_coin_num.toFixed(2)}\n`);
                }
                if (player_n_coin_data.type == "活期存法" && total_time >= 3) {
                    CheckNCoinForm.button("立即取出并结算N币")
                }
                break;
            case "complete":
                CheckNCoinForm.body(`§6您当前的N币数据如下：\n\n` +
            ...
            case "finish":
                ...
                `§e当前持有类型：§c${player_n_coin_data.type}\n` +
                `§e当前持有数量：§c${player_n_coin_data.now_coin_num.toFixed(2)}\n` +
                `§e结算消耗时间：§c${player_n_coin_data.save_time}h\n`)
                for (let i = 0; i < player_n_coin_data.change_log.length; i++) {
                    CheckNCoinForm.divider();
                    CheckNCoinForm.label(`§e第§c${i+1}§e次结算后数量： ` +
                    `§c${player_n_coin_data.change_log[i].after_coin_num.toFixed(2)}\n`);
                }
                CheckNCoinForm.button("取出并结算N币")
                break;
            case "break":
                CheckNCoinForm.body(`§6您当前的N币数据如下：\n\n` +
                `§e当前持有状态：§c爆仓\n` +
                ...
                `§e当前持有类型：§c${player_n_coin_data.type}\n` +
                `§e当前持有数量：§c${player_n_coin_data.now_coin_num.toFixed(2)}\n` +
                `§e爆仓消耗时间：§c${((player_n_coin_data.change_log.length * 12) / 60).toFixed(2)}h\n`)
                for (let i = 0; i < player_n_coin_data.change_log.length; i++) {
                    CheckNCoinForm.divider();
                    CheckNCoinForm.label(`§e第§c${i+1}§e次结算后数量： ` +
                    `§c${player_n_coin_data.change_log[i].after_coin_num.toFixed(2)}\n`);
        ...
        CheckNCoinForm.divider();
        CheckNCoinForm.button("返回上一页")
        CheckNCoinForm.show(player).then((response) => {
            if (response.canceled) return;
            switch (player_n_coin_data.status) {
                case "normal":
                    if (player_n_coin_data.type == "活期存法") {
                        if (response.selection == 0) {
                            //活期开始结算
                            this.CheckNCoinSub(player);
                            return;
                        }
                        if (response.selection == 1) {
                            this.Main(player);
    ...
    CheckNCoinSub(player, insurance) {
        let player_n_coin_data = JSON.parse(world.getDynamicProperty("n_coin_data"))[player.id];
        let can_get_money = Number((player_n_coin_data.now_coin_num * n_coin).toFixed(0));
        let use_insurance = false;
        if (player_n_coin_data.now_coin_num / player_n_coin_data.buy_coin_num < 0.5 && player_n_coin_data.insurance) use_insurance = true;
        if (use_insurance) can_get_money = Number((player_n_coin_data.buy_coin_num * n_coin * 0.5).toFixed(0));
        const CheckNCoinForm = new ActionFormData()
        .title(`N币结算确定`)
        .body(`§6您确定要取出并结算您的N币吗？`)
        .label(`§e按今日N币价格§c${n_coin}§e金币每个计算`)
        .label(`§e您原始持有N币数量为§c${player_n_coin_data.buy_coin_num.toFixed(2)}§e个`)
        .label(`§e您当前持有N币数量为§c${player_n_coin_data.now_coin_num.toFixed(2)}§e个`)
        if (use_insurance) {
            CheckNCoinForm.label(`§e美联储保障您本次交易，当前结束交易将获得§c${can_get_money}§e金币`)
        } else {
            CheckNCoinForm.label(`§e当前结束交易将获得§c${can_get_money}§e金币`)
        }
        CheckNCoinForm.button("确认取出并结算")
        .button("取消结算进程")
        .show(player).then((response) => {
            if (response.canceled) return;
            if (response.selection == 0) {
                //取出并结算N币
                world.scoreboard.getObjective(cfg.MoneyScoreboardName).addScore(player, Number((can_get_money).toFixed(0)));
                player.sendMessage(` §e您已成功取出并结算您的N币，获得金币§c${can_get_money}`);
                n_coin_data[player.id].status = "complete";
                world.setDynamicProperty("n_coin_data", JSON.stringify(n_coin_data));
                // log(JSON.stringify(n_coin_data));
            }
