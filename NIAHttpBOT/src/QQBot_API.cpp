...
{
	//启动时向指定qq群发送消息
	//cli.Post("/send_group_msg", "{\"group_id\":724360499,\"message\":\"Nia-HttpBot已成功连接!\"}", "application/json");
	...
	if (!player_data_file) {
		...
		//向指定qq群发送消息
		cli.Post("/send_group_msg", "{\"group_id\":724360499,\"message\":\"player_data.json文件不存在，已自动创建！\"}", "application/json");
	}

	//检测特定qq群，开头为“#”的消息
	svr.Post("/qqEvent", [&cli](const httplib::Request& req, httplib::Response& res) {
		//INFO(X("QQ接收的数据 ") << req.body);
		//解析字符串并创建一个json对象
		...
		if (post_type == "message") {
			...
			if (message[0] == '#') {
				...
				//未知指令，发送相关提示
				cli.Post("/send_group_msg", "{\"group_id\":" + group_id + ",\"message\":\"未知指令，请发送#help获取指令帮助！\"}", "application/json");
			}
			return ;
		}
		//处理通知事件
		if (post_type == "notice") {
			//后续接入成员变动，自动禁用玩家白名单，进入群聊自动解禁
			return ;
		}
		//处理请求事件
